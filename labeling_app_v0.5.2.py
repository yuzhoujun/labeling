...
功能说明:
    体验优化与问题修复版本，重点增强导出的可视化反馈。
    主要功能:
    1. 导出系统: 增加多线程进度条对话框，支持实时日志与取消操作。
    2. 列表交互: 修复类别列表拖拽排序的逻辑缺陷。
    3. 菜单增强: 右键菜单增加“全选”功能；类别列表双击编辑修改为仅右键修改。

使用说明:
    1. 运行: python labeling_app_v0.5.2.py
    2. 导出: 点击导出按钮将自动弹出进度窗口。

更新日志 (v0.5.2):
    [新增] 支持直接读取与修改大型 COCO 格式文件 (export_coco.json)。
    [新增] 标注列表/画布右键菜单增加“修改类别”功能，支持批量修改。
    [新增] 类别列表、标注列表、图片区域右键菜单增加“全选”按钮。
    [新增] 自动迁移 Class 配置
    [优化] COCO 格式加载性能优化（构建索引），解决大文件卡顿问题。
    [优化] 标注列表支持 Ctrl+A 全选快捷键。
    [优化] 支持 Delete 全选快捷键。
    [优化] 类别列表点击空白区域取消选中，并重置“快速标注”状态。
    [优化] 切换“工作格式”或“标签目录”时自动刷新画布标注。
    [修改] 禁用类别列表双击修改名称功能（防误触），保留右键菜单修改入口。
    [修复] 移除残留的标注列表单选模式代码，确保多选功能正常。
    [修复] 导出错误与进度页面卡顿与警告。
    [修复] XML 标注不显示问题

打包指令:
    pyinstaller --noconsole --onefile --icon=assets/app_icon_colored.png --name="LabelingTool_v0.5.2" --add-data "assets;assets" labeling_app_v0.5.2.py
...
from PyQt5.QtCore import Qt, QSize, QPoint, QPointF, QRectF, QRect, QEvent, QDate, QTime, QDateTime, QThread, pyqtSignal
...
class AnnotationCanvas(QWidget):
    ...
    def mouseReleaseEvent(self, event):
        if self._state == self.STATE_PANNING:
            ...
            ts = get_timestamp()
            for idx in self.selected_indices:
                 self._shapes[idx]['updated_at'] = ts
                 # Clamp final just in case?
                 r = self._shapes[idx]['rect']
                 x, y, w, h = r.x(), r.y(), r.width(), r.height()
                 x = max(0.0, min(1.0-w, x))
                 y = max(0.0, min(1.0-h, y))
                 self._shapes[idx]['rect'] = QRectF(x, y, w, h)
...
class AnnotationApp(QWidget):
    ...
    def show_annotation_context_menu(self, pos, from_canvas=False):
        ...
        if indices:
            act_del = menu.addAction("删除选中 (Delete)")
    ...
    def delete_selected_annotations(self):
        ...
        if not DarkDialogHelper.ask_yes_no(self, "删除确认", f"确定删除选中的 {len(indices)} 个标注吗？"):
            return
            
        current_shapes = self.canvas.get_shapes()
        for i in indices:
            if 0 <= i < len(current_shapes):
                current_shapes.pop(i)
    ...
    def keyPressEvent(self, event):
        ...
        # If focus is on Tables, eventFilter handles it.
        # If focus is elsewhere (Canvas), we handle it here.
        if key == Qt.Key_Delete:
            if not isinstance(self.focusWidget(), (QLineEdit, QTextEdit)):
               self.delete_selected_annotations()
