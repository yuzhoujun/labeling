...
使用说明:
    1. 运行: python labeling_app_v1.3.1.py
    2. 导出: 点击导出按钮将自动弹出进度窗口。
    
更新日志 (v1.3.1):
    [修复] [labeling_app_v1.3py](labeling_app_v1.3py) 中 `DraggableTableWidget` 的 `dropEvent`：不再使用行号做加减法计算 `logical_to_idx`，改为基于提取相邻逻辑项（`id_above_v` 和 `id_below_v`）的真实 ID 来确定最终 `self.class_data` 的插入位置。
