...
使用说明:
    1. 运行: python labeling_app_v0.5.3.py
    2. 导出: 点击导出按钮将自动弹出进度窗口。

更新日志 (v0.5.3):
    [新增] 高级筛选对话框：支持 AND/OR 多条件逻辑组合（UI优化：首行对齐、极致窄化逻辑框）。
    [新增] 高级筛选对话框：新增按钮改为绿色圆角矩形背景图标，提升视觉层级。
    [优化] 类别的提示框（及其他输入框）下拉菜单样式，增加绿色状态指示块。
    [优化] 统一深色模式下输入对话框的视觉体验。
    [修复] 修复筛选对话框底部按钮视觉截断问题，并校正图标居中对齐。

打包指令:
    pyinstaller --noconsole --onefile --icon=assets/app_icon_colored.png --name="LabelingTool_v0.5.3" --add-data "assets;assets" labeling_app_v0.5.3.py
