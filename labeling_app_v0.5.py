...
功能说明:
    交互体验的重大重构，引入现代化的多选与剪贴板系统。
    主要功能:
    1. 全局多选: 画布 Ctrl+拖拽框选、Ctrl+点击多选；列表 Ctrl/Shift 多选。
    2. 联动同步: 画布与标注列表的选择状态实时双向同步。
    3. 剪贴板: 支持 Ctrl+C/V/X 复制粘贴标注数据。
    4. 批量操作: 支持批量移动、删除标注框。

使用说明:
    1. 运行: python labeling_app_v0.5.py
    2. 多选: 按住 Ctrl 在画布点击或拖拽。
    3. 复制: 选中标注后 Ctrl+C，在同图或不同图 Ctrl+V。

更新日志 (v0.5):
    [重构] 核心交互逻辑，支持复杂的 Selected Indices 集合管理。
    [新增] 画布 Rubber Band (橡皮筋) 框选功能。
    [新增] 跨图片/跨应用的文本格式剪贴板支持。
    [新增] 右键菜单支持“删除选中”、“复制”、“剪切”。
    [优化] 手柄调整逻辑，点击单手柄自动切换单选模式。

打包指令:
    pyinstaller --noconsole --onefile --icon=assets/app_icon_colored.png --name="LabelingTool_v0.5" --add-data "assets;assets" labeling_app_v0.5.py
"""
