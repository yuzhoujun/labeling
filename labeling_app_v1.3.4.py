...
使用说明:
    1. 运行: python labeling_app_v1.3.4.py
    2. 导出: 点击导出按钮将自动弹出进度窗口。
    
更新日志 (v1.3.4):
    [优化] 在 `AnnotationCanvas.mouseMoveEvent`（平移结束或过程）和 `wheelEvent`（缩放）中，手动构造 `BaseAction` 并调用 `_log_action(action, "VIEW")` 记录 Level 0 视图行为但不推入撤销栈。
    [优化] 画布局部平移（`mouseMoveEvent`）频率极高，在右键 `mouseReleaseEvent` 中只记录一次以避免操作池被瞬间刷屏
    [优化] 对于其他软件优化：把保存、解析等比较耗时的环节统一迁移到异步任务（QThread）中避免大图 / 大文件时卡顿

重点记录：将保存、加载文件、解析等迁移至 QThread
    [方案建议] 在大规模数据管理或读取高清图时不可避免会出现 UI 冻结（通常是因为 I/O 阻塞）。要解决这个问题，可以通过抽取多线程 Worker 来实现：
    [实现思路] 类似于当前我们存在的 ExportWorker，可以提供一个 SaveWorker 或 LoadImageWorker。
    [难点] PyQt 会严格要求所有界面元素的渲染（如更新列表、重新绘画 self.update()）必须留在主线程。您需要把 cv2.imread()、json.load() / xml.etc.ElementTree() 等纯 CPU 及磁盘读写抽出并放入 QThread 获取结果字典；然后在 finished 的槽函数中发送 Signal 到主线程去真正调用 canvas.set_pixmap() 和渲染表格内容。
    [重点][代价] 对于一个单文件脚本，引入过多线程需要妥善处理线程间的锁定（锁列表以避免同时修饰出现崩溃），开发复杂度较高。如果当前未出现极其严重的性能瓶颈，这个优化是可以暂缓或在 v2.0 重构时配合模块化拆分来实施。
...
class ActionPool:
    ...
    def _log_action(self, action, op_type="EXECUTE"):
        colors = {
            ActionLevel.VIEW: "#808080",     # 灰色
            ActionLevel.MINOR: "#cccccc",    # 浅灰色
            ActionLevel.NORMAL: "#ffffff",   # 白色
            ActionLevel.MAJOR: "#ffff00",    # 黄色
            ActionLevel.CRITICAL: "#ff0000"  # 红色
        }
        color = colors.get(action.level, "#ffffff")
        timestamp = datetime.now().strftime('%H:%M:%S')
        log_entry = f'<span style="color:{color};">[{timestamp}] {op_type} | Level: {action.level} | Desc: {action.description} | Context: {self.current_context}</span>'
...
class CompositeAction(BaseAction):
    def __init__(self, description, actions):
        # 综合考虑所有子动作中最高级别的 level
        level = max((a.level for a in actions), default=ActionLevel.NORMAL)
        super().__init__(description, level=level)
...
class AddClassAction(BaseAction):
    def __init__(self, main_app, class_dict):
        super().__init__(f"添加类别: {class_dict.get('name', '未命名')}", level=ActionLevel.NORMAL)
        self.main_app = main_app
    ...
    def do(self):
        self.previous_class_index = self.main_app.current_class_index
        self.added_index = len(self.main_app.class_data)
        self.class_dict["id"] = self.added_index  # Update ID based on length
        self.main_app.class_data.append(self.class_dict)
        self.main_app.save_classes_json()
        self.main_app.refresh_class_table()
        
        # 恢复/更新 current_class_index 与表格选中状态
        self.main_app.current_class_index = self.added_index
        if self.added_index >= 0:
            self.main_app.class_table.selectRow(self.added_index)

    def undo(self):
        if 0 <= self.added_index < len(self.main_app.class_data):
            self.main_app.class_data.pop(self.added_index)
            # Re-sync IDs for any class after added_index if needed
            for i, c in enumerate(self.main_app.class_data):
                c['id'] = i
            self.main_app.save_classes_json()
            self.main_app.refresh_class_table()
            
            # 恢复之前的 current_class_index 与表格选中状态
            self.main_app.current_class_index = self.previous_class_index
            if self.previous_class_index >= 0:
                self.main_app.class_table.selectRow(self.previous_class_index)
            else:
                self.main_app.class_table.clearSelection()

class EditClassAction(BaseAction):
    def __init__(self, main_app, class_index, old_dict, new_dict):
        super().__init__("修改类别", level=ActionLevel.NORMAL)
        self.main_app = main_app
        self.class_index = class_index
        self.old_dict = old_dict.copy()
        self.new_dict = new_dict.copy()

    def do(self):
        if 0 <= self.class_index < len(self.main_app.class_data):
            self.main_app.class_data[self.class_index].update(self.new_dict)
            self.main_app.save_classes_json()
            self.main_app.refresh_class_table()
            self.main_app.canvas.update()
            self.main_app.refresh_annotation_table()
            # 保持选中被修改的行
            self.main_app.current_class_index = self.class_index
            self.main_app.class_table.selectRow(self.class_index)

    def undo(self):
        if 0 <= self.class_index < len(self.main_app.class_data):
            self.main_app.class_data[self.class_index].update(self.old_dict)
            self.main_app.save_classes_json()
            self.main_app.refresh_class_table()
            self.main_app.canvas.update()
            self.main_app.refresh_annotation_table()
...
class DeleteClassAction(BaseAction):
    def __init__(self, main_app, class_index):
        super().__init__("删除类别", level=ActionLevel.MAJOR)
        self.main_app = main_app
        self.class_index = class_index
        self.deleted_class = None
        self.previous_class_index = -1
        self.original_shapes_state = []
        self.remap_handled = False

    def do(self):
        import copy
        self.previous_class_index = self.main_app.current_class_index
        # 深拷贝当前画布形状，以便撤销时完美恢复
        self.original_shapes_state = copy.deepcopy(self.main_app.canvas.get_shapes())

        if 0 <= self.class_index < len(self.main_app.class_data):
            self.deleted_class = self.main_app.class_data.pop(self.class_index)
            # Re-sync IDs
            for i, c in enumerate(self.main_app.class_data):
                c['id'] = i
            self.main_app.save_classes_json()
            ...
            # 删除后清空或更新 current_class_index
            self.main_app.current_class_index = -1
            self.main_app.class_table.clearSelection()

            # 处理级联的标注框：剔除被删除类的标注，其余后续类别 index 前移
            shapes = self.main_app.canvas.get_shapes()
            new_shapes = []
            changed = False
            for s in shapes:
                if s['class_index'] == self.class_index:
                    changed = True  # 该框也连带删除
                    continue
                elif s['class_index'] > self.class_index:
                    s_new = s.copy()
                    s_new['class_index'] -= 1
            ...
            if changed:
                self.main_app.canvas.set_shapes(new_shapes)
                self.main_app.save_current_annotations()
                self.main_app.refresh_annotation_table()
                self.main_app.canvas.update()
                self.remap_handled = True

    def undo(self):
        if self.deleted_class is not None:
            self.main_app.class_data.insert(self.class_index, self.deleted_class)
            # Re-sync IDs
            for i, c in enumerate(self.main_app.class_data):
                c['id'] = i
            self.main_app.save_classes_json()
            ...
            # 还原被连带修改/删除的标注框状态
            if self.remap_handled:
                import copy
                self.main_app.canvas.set_shapes(copy.deepcopy(self.original_shapes_state))
                self.main_app.save_current_annotations()
                self.main_app.refresh_annotation_table()
                self.main_app.canvas.update()
...
class AddShapeAction(BaseAction):
    ...
    def do(self):
        ...
        self.main_app.refresh_annotation_table()
        # 更新画布中高亮显示的框
        self.main_app.canvas.selected_shape_index = self.added_index
        self.main_app.canvas.update()
    ...
    def undo(self):
        ...
        if 0 <= self.added_index < len(shapes):
            ...
            self.main_app.save_current_annotations()
            self.main_app.refresh_annotation_table()
            # 撤销后清除画布选中状态，避免索引越界或错位
            self.main_app.canvas.selected_shape_index = -1
            self.main_app.canvas.update()
...
class DraggableTableWidget(QTableWidget):
    ...
    def dropEvent(self, event):
        ...
        # 4. 基于真实相连的逻辑项 ID 来确定实际插入位置，摒弃视觉行号计算防止坐标越出边界
        target_reference_id = id_below_v if sort_order == Qt.AscendingOrder else id_above_v
        
        logical_to_idx = -1
        if target_reference_id != -1:
            for i, c in enumerate(self.main_app.class_data):
                if c.get("id", -1) == target_reference_id:
                    logical_to_idx = i
                    break
                    
        if logical_to_idx == -1: # 此情况涵盖了升序放置在末尾 或 降序放置在顶端（均为逻辑数据列表的末尾）
            logical_to_idx = len(self.main_app.class_data)
...
class AnnotationCanvas(QWidget):
        ...
        if self._state == self.STATE_PANNING:
            if self._pan_start_pos is not None:
                ...
                if dist < 5:
                     ...
                     action = BaseAction("画布平移", level=ActionLevel.VIEW)
                     self.main_tab.action_pool._log_action(action, "VIEW")
    ...
    def wheelEvent(self, event):
        ...
        # 为了避免瞬间大量生成缩放日志，将其记录为 VIEW 级动作不入栈
        action_name = "画布放大" if zoom_in else "画布缩小"
        action = BaseAction(action_name, level=ActionLevel.VIEW)
        self.main_tab.action_pool._log_action(action, "VIEW")
...
class AnnotationApp(QWidget):
    ...
    def show_action_pool_log(self):
        ...
        # 将日志通过 <br> 拼接并设置为 HTML 渲染 (支持设定的颜色跨度)
        logs_html = "<br>".join(self.action_pool.action_log)
        text_edit.setHtml(logs_html)
    ...
    def show_class_context_menu(self, pos):
        ...
        add_action.triggered.connect(lambda: self.add_new_class_dialog()) 
    ...
    def add_new_class_dialog(self, execute_immediately=True):
        ...
        if ok and name:
            name = name.strip()
            if not name: return -1 if execute_immediately else (None, -1)
            for i, cls in enumerate(self.class_data):
                if cls['name'] == name:
                    return i if execute_immediately else (None, i)
            color = DarkDialogHelper.get_color(self)
            if color.isValid():
                new_idx = len(self.class_data)
                class_dict = {"id": new_idx,
                              "name": name,
                              "color": [color.red(), color.green(), color.blue()],
                              "updated_at": get_timestamp()}
                action = AddClassAction(self, class_dict)
                if execute_immediately:
                    self.action_pool.execute(action)
                    return action.added_index
                else:
                    return action, new_idx
        return -1 if execute_immediately else (None, -1)
    ...
    def edit_class_name(self, idx):
        ...
        if ok and name:
            old_dict = self.class_data[idx].copy()
            new_dict = self.class_data[idx].copy()
            new_dict['name'] = name
            new_dict['updated_at'] = get_timestamp()
            
            action = EditClassAction(self, idx, old_dict, new_dict)
            self.action_pool.execute(action)
    ...
    def edit_class_color(self, idx):
        ...
        if color.isValid():
            old_dict = self.class_data[idx].copy()
            new_dict = self.class_data[idx].copy()
            new_dict['color'] = [color.red(), color.green(), color.blue()]
            new_dict['updated_at'] = get_timestamp()
    ...
    def delete_class(self, idx):
        if DarkDialogHelper.ask_yes_no(self, "确认删除", "删除此类别可能影响现有标注，确定吗？"):
            action = DeleteClassAction(self, idx)
            self.action_pool.execute(action)
    ...
    def add_new_shape(self, rect_normalized):
        ...
        if cls_idx == -1:
            class_action_or_none, new_idx_or_none = self.add_new_class_dialog(execute_immediately=False)
            if new_idx_or_none != -1:
                cls_idx = new_idx_or_none
                class_action = class_action_or_none
            else:
                return

        # Prepare AddShapeAction
        # Since class_action might not have been executed yet, we set the index directly
        shape_data = {'class_index': cls_idx, 'rect': rect_normalized, 'updated_at': get_timestamp()}
        shape_action = AddShapeAction(self, shape_data)
        
        if class_action:
            # We must link the added class index logic after class action runs
            # Because added_index inside AddClassAction isn't known yet until do()
            # Wait, our add_new_class_dialog assumes id is len(class_data)
            # which is statically known (new_idx_or_none).
            comp = CompositeAction("新建类别并画框", [class_action, shape_action])
            self.action_pool.execute(comp)
        else:
            self.action_pool.execute(shape_action)
            
        if cls_idx != -1 and not class_action:
            self.class_table.selectRow(cls_idx)
    ...
    def modify_selected_annotations_class(self, indices=None):
        ...
        if ok and name:
            ...
            if new_cls_idx == -1:
                color = DarkDialogHelper.get_color(self)
                if color.isValid():
                    class_dict = {"id": len(self.class_data),
                                  "name": name,
                                  "color": [color.red(), color.green(), color.blue()],
                                  "updated_at": get_timestamp()}
                    add_class_act = AddClassAction(self, class_dict)
                    actions_to_execute.append(add_class_act)
                    new_cls_idx = len(self.class_data)
                else: 
                    return
            ...
            if new_cls_idx != -1:
                ...
                for idx in indices:
                    if 0 <= idx < len(current_shapes):
                        old_val = current_shapes[idx]['class_index']
                        if old_val != new_cls_idx:
                            actions_to_execute.append(ModifyShapeClassAction(self, idx, old_val, new_cls_idx))

                if actions_to_execute:
                    if len(actions_to_execute) == 1:
                        self.action_pool.execute(actions_to_execute[0])
                    else:
                        comp = CompositeAction("修改选中框类别", actions_to_execute)
                        self.action_pool.execute(comp)
