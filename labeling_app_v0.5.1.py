...
使用说明:
    1. 运行: python labeling_app_v0.5.1.py
    2. 导出: 点击导出按钮将自动弹出进度窗口。

更新日志 (v0.5.1):
    [新增] ExportProgressDialog 导出进度窗口，显示进度条与详情日志。
    [优化] 导出任务移至后台线程 (QThread) 执行，防止界面卡死。
    [修复] 类别列表 dropEvent 逻辑，修复拖拽排序导致列表项丢失的问题。

打包指令:
    pyinstaller --noconsole --onefile --icon=assets/app_icon_colored.png --name="LabelingTool_v0.5.1" --add-data "assets;assets" labeling_app_v0.5.1.py
"""
...
class ExportWorker(QThread):
    ...
    def run(self):
        ...
        for i, f in enumerate(self.files):
            ...
            try:
                # Load
                shapes, _ = self.func_load(f, self.fmt)
...
class ExportProgressDialog(QDialog):
    def __init__(self, parent, worker):
        ...
        self.detail_chk.setChecked(False)
    ...
    def toggle_details(self, state):
        self.log_text.setVisible(state == Qt.Checked)
...
class AnnotationApp(QWidget):
    ...
    def delete_selected_classes_logic(self):
        ...
        if not DarkDialogHelper.ask_yes_no(self, "删除确认", f"确定删除选中的 {len(ids_to_delete)} 个类别吗？\n注意：这将移除列表中的条目，且可能会造成ID不连续或Shift。"):
             return

        # Remove from class_data
        self.class_data = [c for c in self.class_data if c['id'] not in ids_to_delete]
