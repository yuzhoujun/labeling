...
功能说明:
    v0.4 系列的稳定版，完善了筛选系统和 UI 细节。
    主要功能:
    1. 筛选高亮: 当列存在筛选条件时，表头图标高亮提醒。
    2. 深度时间筛选: 支持年月日时分秒的精确筛选。
...
更新日志 (v0.4.2):
    [优化] 筛选图标在激活状态下显示高亮色。
    [增强] 时间筛选功能支持 6 维度 (年/月/日/时/分/秒) 精确匹配。
    [修复] 解决类别列表拖拽排序时的部分逻辑冲突。

打包指令:
    pyinstaller --noconsole --onefile --icon=assets/app_icon_colored.png --name="LabelingTool_v0.4.2" --add-data "assets;assets" labeling_app_v0.4.2.py
"""
