...
使用说明:
    1. 运行: python labeling_app_v1.3.3.py
    2. 导出: 点击导出按钮将自动弹出进度窗口。

### Steps
5. 在 `AnnotationCanvas.mouseMoveEvent`（平移结束或过程）和 `wheelEvent`（缩放）中，手动构造 `BaseAction` 并调用 `_log_action(action, "VIEW")` 记录 Level 0 视图行为但不推入撤销栈。
### Further Considerations
1. 画布局部平移（`mouseMoveEvent`）频率极高，建议在右键 `mouseReleaseEvent` 中只记录一次以避免操作池被瞬间刷屏？
2. 对于其他软件优化：是否需要把保存、解析等比较耗时的环节统一迁移到异步任务（QThread）中避免大图 / 大文件时卡顿？
    
更新日志 (v1.3.3):
    [修复] `AnnotationApp.show_action_pool_debug`：为对话框增加 `.setStyleSheet("background-color: #1e1e1e; color: #cccccc;")` 赋予深色主题。
    [增强] `ActionPool._log_action` 与 UI 显示：使用带颜色的 HTML 标签格式化日志（如 VIEW 级灰色、NORMAL 级白色、MAJOR 级黄色、CRITICAL 级红色），并将 `QTextEdit` 变更为支持 Rich Text 的渲染。
