...
使用说明:
    1. 运行: python labeling_app_v1.3.2.py
...
更新日志 (v1.3.2):
    [修复] `AnnotationApp.show_class_context_menu` 等处的 `add_new_class_dialog` 绑定：将 `add_action.triggered.connect(self.add_new_class_dialog)` 更改为 `lambda: self.add_new_class_dialog()`，防止 Qt 传递布尔值导致 `execute_immediately=False`。
