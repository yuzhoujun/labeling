...
功能说明:
    引入高级筛选系统，支持对海量数据的快速检索。
    主要功能:
    1. 高级筛选: 支持对表格的各列（文本、数值、时间）进行多维筛选。
    2. UI 升级: 自定义表头绘制，集成筛选状态图标。

使用说明:
    1. 运行: python labeling_app_v0.4.py
    2. 筛选: 右键点击表格标题栏，选择“筛选设置”。

更新日志 (v0.4):
    [新增] 高级筛选对话框，支持等于、包含、大于小于等逻辑。
    [新增] 自定义 QHeaderView，显示排序与筛选漏斗图标。
    [优化] 深度适配 Dark Theme 的表格样式。

打包指令:
    pyinstaller --noconsole --onefile --icon=assets/app_icon_colored.png --name="LabelingTool_v0.4" --add-data "assets;assets" labeling_app_v0.4.py
"""
...
class DraggableTableWidget(QTableWidget):
    ...
    def dropEvent(self, event):
        ...
        if target_row == -1: target_row = self.rowCount() - 1
        ...
        # 允许拖拽的条件：
        # 1. 没有开启筛选
        # 2. 排序处于默认的 Class ID 列 (0)
        # 注意: QTableWidget 默认会有 SortIndicator, 我们需要检查 Section
        
        is_filtered = len(self.current_filter_list) > 0
        ...
        # 如果按照非ID列排序，禁止拖动
        # 假设列 0 是 ID 列
        if is_sorting and sort_col != 0 and sort_col != -1:
             DarkDialogHelper.show_warning(self, "操作限制", "请先按类别序号列排序后再进行拖拽操作。")
             event.ignore()
             return

        # 映射 Visual Row 到 Data Index
        # 如果是升序 (Ascending): Data Index = Visual Row
        # 如果是降序 (Descending): Data Index = RowCount - 1 - Visual Row
        # 注意: 只有在 Class ID (Col 0) 也就是类别标签列排序下，这个映射才成立 (因为 ID即Index)
        
        N = self.rowCount()
        sort_order = header.sortIndicatorOrder() if header else Qt.AscendingOrder # type: ignore
        
        src_vis = self.drag_start_row
        tgt_vis = target_row
        
        # 1. 计算 Visual Insert Target (视觉上插入到哪里?)
        drop_pos = self.dropIndicatorPosition()
        
        if drop_pos == QAbstractItemView.OnItem:
             s_dat = src_vis if sort_order == Qt.AscendingOrder else N - 1 - src_vis # type: ignore
             t_dat = tgt_vis if sort_order == Qt.AscendingOrder else N - 1 - tgt_vis # type: ignore
             
             if DarkDialogHelper.ask_yes_no(self, "交换类别序号", f"确定交换 类别序号 {s_dat} 和 类别序号 {t_dat} 吗？\n这将影响所有相关联的标注。"):
                 self.main_app.swap_class_ids(s_dat, t_dat)
                 event.accept()
             return

        # 移动操作 (Insert Logic using "Middle Value" Strategy)
        # 获取 Target Row 周围的两个 item 的 Data Value (ID)
        # Target Row (tgt_vis) 是插入的目标视觉行
        # 如果 drop_pos == OnItem -> 已被 Swap 处理
        # 如果 Drop Below Item -> 目标是 tgt_vis 的下方 -> 也就是 Before item at (tgt_vis + 1)
        # 如果 Drop On Viewport -> Append -> 目标是 End
        
        # 确定 Visual "Below" Item (插入点之后的 Item) 和 "Above" Item (插入点之前的 Item)
        vis_row_after = -1
        vis_row_before = -1
        
        if drop_pos == QAbstractItemView.BelowItem:
            vis_row_before = tgt_vis
            vis_row_after = tgt_vis + 1
        elif drop_pos == QAbstractItemView.OnViewport:
             # Append to visual end
             vis_row_before = N - 1
             vis_row_after = N # Out of bounds
        else: # On/Above Item (Above logic is default for QAbs..ItemMove if not OnItem?)
             # Actually, if not OnItem and not BelowItem, it's typically "AboveItem" (0x0...1?) or just at pos.
             # In standard drag drop, if indicator is line above, it's insert before.
             vis_row_after = tgt_vis
             vis_row_before = tgt_vis - 1

        # Retrieve the Data IDs (UserRole) from these visual rows
        val_before = None
        val_after = None
        
        if 0 <= vis_row_before < N:
            item_b = self.item(vis_row_before, 0)
            if item_b: val_before = float(item_b.data(Qt.UserRole)) # type: ignore
            
        if 0 <= vis_row_after < N:
            item_a = self.item(vis_row_after, 0)
            if item_a: val_after = float(item_a.data(Qt.UserRole)) # type: ignore
            
        # Calculate New "Float Rank"
        # We need the source ID too
        src_item_cell = self.item(src_vis, 0)
        src_id = src_item_cell.data(Qt.UserRole) if src_item_cell else -1 # type: ignore
        if src_id == -1: return

        # Calc target index logic
        # Descending view (High -> Low). Interpolating between 3 and 2 gives 2.5
        # We need target index 3 (Higher index). So Ceil.
        # Ascending view (Low -> High). Interpolating between 2 and 3 gives 2.5.
        # We need target index 2 (Lower index). So Floor.
        ...
        def get_val(r):
            if 0 <= r < N:
                it = self.item(r, 0)
                if it: return float(it.data(Qt.UserRole)) # type: ignore
            return None
            
        val_b = get_val(vis_row_before)
        val_a = get_val(vis_row_after)
        
        is_desc = (sort_order == Qt.DescendingOrder) # type: ignore
        
        # Case 1: Between two items (Normal insertion in list)
        if val_b is not None and val_a is not None:
            mid = (val_b + val_a) / 2.0
            if is_desc: target_idx = math.ceil(mid)
            else: target_idx = math.floor(mid)
            
        # Case 2: Only Before (e.g. End of list)
        elif val_b is not None:
             # Dropped after B
             target_idx = 0 if is_desc else len(self.main_app.class_data)
                 
        # Case 3: Only After (e.g. Top of list)
        elif val_a is not None:
             # Dropped before A.
             # Desc: Before High. -> New Highest. -> Target = N.
             # Asc: Before Low. -> New Lowest. -> Target = 0.
             target_idx = len(self.main_app.class_data) if is_desc else 0
             
        # Fallback
        if target_idx == -1: target_idx = 0
        
        self.main_app.move_class_id_direct(src_id, int(target_idx))
        event.accept()
...
class AnnotationApp(QWidget):
    ...
    def load_classes(self):
        ...
        for i, c in enumerate(self.class_data):
            ...
            c['id'] = i # Sync ID with list index
    ...
    def refresh_class_table(self):
        ...
        for i, cls in enumerate(self.class_data):
            ...
            # Only Name col gets background color in standard way
            item_name.setBackground(QColor(*c))
            if lum < 128: item_name.setForeground(QColor(255, 255, 255))
            else: item_name.setForeground(QColor(0, 0, 0))
        ...
        if ok and new_id != old_idx:
            if new_id < len(self.class_data):
                reply = DarkDialogHelper.ask_yes_no_cancel(self, "类别序号冲突", 
                    ...
                    "选择【是】替换(交换)位置，选择【否】插入到该位置(后移)，【取消】取消操作。")
    ...
    def move_class_id_direct(self, from_idx, to_idx):
        """ 
        Direct Move: Update List -> Save JSON -> Reload Full State.
        This provides the highest robustness against index/data corruption.
        """
        if from_idx == to_idx: return
        
        # 0. Safety Check
        if not (0 <= from_idx < len(self.class_data)): return
        
        # 1. Update List Data in Memory
        item = self.class_data.pop(from_idx)
        item['updated_at'] = get_timestamp()
        
        # Adjust insert index
        if to_idx > len(self.class_data): to_idx = len(self.class_data)
        if to_idx < 0: to_idx = 0
            
        self.class_data.insert(to_idx, item)
        final_idx = to_idx 

        # 2. Update Annotations using range logic based on from/final idx
        # This memory update is needed before saving/reloading so we don't lose annotation links
        shapes = self.canvas.get_shapes()
        for s in shapes:
            idx = s['class_index']
            # Logic same as before:
            if from_idx < final_idx: # Moved Down
                 # (from, final] shifted down -1
                 if idx == from_idx: s['class_index'] = final_idx
                 elif from_idx < idx <= final_idx: s['class_index'] = idx - 1
            elif from_idx > final_idx: # Moved Up
                 # [final, from) shifted up +1
                 if idx == from_idx: s['class_index'] = final_idx
                 elif final_idx <= idx < from_idx: s['class_index'] = idx + 1
        
        # 3. Save & Reload
        # Saving will write the new order.
        # Loading will read the new order (and assign new IDs 0..N).
        self.save_classes_json()
        self.load_classes() # This triggers refresh_class_table()
        
        # 4. Refresh annotations table/canvas
        self.refresh_annotation_table()
        self.canvas.update()

    def move_class_id(self, from_idx, to_idx):
        # Legacy/Internal method - redirect to direct
        self.move_class_id_direct(from_idx, to_idx)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(" 数据标注工具 v0.4") 
